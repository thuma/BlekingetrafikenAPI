...
if(is_file($namelist) == FALSE)
	...
	for($i = 0; $i < count($bokstaver); $i++)
		...
		while($newdata)
			...
			$newdata = FALSE;
			$doc = new DOMDocument();
			@$doc->loadHTML(file_get_contents('http://www.reseplaneraren.blekingetrafiken.se/v2/indexes.aspx?optType=0&selKommun=0&sLetter='.urlencode(utf8_decode($bokstaver[$i])).'&iPage='.$page.'&Language=se&optFrTo=0&TNSource=BLEKINGE'));
...
if(is_file($idlist) == FALSE)
	...
	foreach($all as $key => $name)
		{
		$url = 'http://www.reseplaneraren.blekingetrafiken.se/v2/rpajax.aspx?net=BLEKINGE&lang=se&letters='.urlencode(utf8_decode($name->cleanname));
